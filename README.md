...
`recipes/sdxl-lora.yaml` のimageは公開GHCR digestへ置換してから利用する。タグやプレースホルダーはCLIが拒否する。入力名・成果物名・hard limit precedenceは [`docs/recipe-schema.md`](docs/recipe-schema.md) に固定している。secretと課金停止を含む運用は [`docs/operations.md`](docs/operations.md) を参照。
