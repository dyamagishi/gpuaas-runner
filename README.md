...
Runpodctl経由のPod作成、SSH/rsync転送、remote runner起動、artifact回収、cleanupまでのCLI経路とMock/unit testsを実装済みです。2026-07-26にRunPod実機でAnimagine XL 4.0をベースに10 stepのSDXL LoRA学習を完走し、safetensors 2本を回収、Pod削除まで確認しました。Vast.ai、musubi-tuner、attach/recover再接続、remoteへのsecret注入、artifact manifestのCLI側SHA照合は未実施/未実装です。`Dockerfile` はsd-scripts commit `37a1cbbc5725ed2a3575506e7bd2001c9908ac92`（SDXL）とCUDA/PyTorch依存を固定する。Anima LoRAは別イメージ `Dockerfile.anima`（sd-scripts v0.11.1 `6721028c79ee85a78b3a06dfd8954dae310a1cce`）と `recipes/anima-lora.yaml`。Docker buildには`CUDA_BASE_IMAGE` repository variable（`@sha256:` 64桁digest）が必須で、placeholderのままでは失敗する。
